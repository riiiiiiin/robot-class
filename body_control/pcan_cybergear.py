...
class CANMotorController:
        ...
        if type == "decode":
            ...
            for f in format_list:
                ...
                if len(s_f) == 2:
                    ...
                    logging.debug("unknown format in format_data(): " + f)
                    return []
            return rdata
            ...
            for i in range(len(format_list)):
                ...
                if len(s_f) == 2:
                    ...
                    logging.debug("unkown format in format_data(): " + f)
                    return []
            if len(rdata) < 4:
    ...
    def send_receive_can_message(self, cmd_mode, data2, data1, timeout=200):
        ...
        try:
            ...
            logging.debug("Failed to send the message.")
            return None, None
    ...
    def parse_received_msg(self, data, arbitration_id):
        ...
        if data is not None:
            ...
            logging.debug("No message received within the timeout period.")
    ...
    def clear_can_rx(self, timeout=10):
        ...
        while True:
            ...
            if received_msg is None:
                break
            logging.debug(f"Cleared message with ID {hex(received_msg.arbitration_id)}")
    ...
    def write_single_param(self, param_name, value):
        ...
        if param_info is None:
            logging.debug(f"Unknown parameter name: {param_name}")
            return
    ...
    def write_param_table(self, param_name, value):
        ...
        if type_code is None:
            logging.debug(f"Unknown parameter type: {param_type}")
            return None, None, None
        ...
        encoded_value = self.format_data(data=[value], format=format, type="encode")
    ...
    def send_motor_control_command(self, torque, target_angle, target_velocity, Kp, Kd):
        ...
        target_velocity_mapped = self._linear_mapping(target_velocity, -30.0, 30.0)
        ...
        received_msg_data, received_msg_arbitration_id = self.send_receive_can_message(
            cmd_mode=cmd_mode, data2=data2, data1=data1
        )
...
if __name__ == "__main__":
    ...
    controller = CANMotorController(bus, motor_id=102, main_can_id=0)
    controller.set_run_mode(controller.RunModes.POSITION_MODE)
    controller.set_0_pos()
    controller.enable()
    input()
    controller.set_motor_position_control(2, 90)
    input()
    controller.set_motor_position_control(2, 0)
    controller.disable()
    bus.shutdown()
